...
int main()
{
    string pt, key, pt_permuted, left0, right0, right1, left1, pt_after_round_1;
    pair<string, string> data = get_data();
    pt = data.first;
    key = data.second;
    // pt_permuted = (initialPertmutation(pt));
    left0 = pt.substr(0, 8);
    right0 = pt.substr(8, 8);
    left1 = right0;
    // R[i] = L[i-1] xor f(R[i-1],Key)
    right1 = (xor_(left0, f(right0, key)));
    pt_after_round_1 = left1 + right1;
    cout << "Text after first round: " << pt_after_round_1 << endl;
    return 0;
}